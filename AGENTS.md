...
本文件（`AGENTS.md`）係 frontend 產品／UX／架構嘅 source of truth，同時包含 agent project rules（見文末）。
...
**每次改 code 都要同步更新本文件相關章節。** 路由、互動、狀態、loading／error、測試範圍有變，就改對應 §。未更新本文件嘅改動當未完成，唔好當做完（見 §14）。
...
- Edit 不可改變 `source`
- 離線時表單仍可填寫，但送出會呼叫 API 失敗並顯示錯誤（見 §8.3）；banner 提示目前離線，不假裝成功、不排隊重試
- 離開 dirty form 前顯示確認
...
- **每次改 code 都已同步更新 `AGENTS.md` 相關章節**。未更新 spec 當未完成
...
- 產品／UX／架構 source of truth 係 repo root 嘅 `AGENTS.md`（本文件）。
- **每次改 code 都要同步更新本文件相關章節**（路由、互動、狀態、loading／error、測試、DoD）。未更新 spec 當未完成。
