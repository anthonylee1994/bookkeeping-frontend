...
Preview 嘅逐筆結果放喺 `parsed_items: [{ parsed, suggested_category_id }]`（receipt 恆一筆；`parsed`／`suggested_category_id` 為第一筆作向後兼容）。覆核 model `previewToReviewValues` 讀第一筆，同一套 `parsedItemToReviewValues`／`scanReviewSchema`／`reviewValuesToInput` 亦被 [§5.10](./5.10-natural-language-entry.md) 多筆拆單嘅批量覆核 drawer 重用。
