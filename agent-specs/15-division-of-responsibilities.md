...
3. AI 辨識由後端呼叫 DeepSeek，結果只作建議，必須經人工覆核才入帳。AI 收支概況（`/summaries/{period}/insight`，只支援 `weekly`／`monthly`）同樣由後端產生：後端先計好 deterministic 數字再交 AI 寫成人話，AI 唔准自行計算；frontend 只顯示結果，唔會接觸 API key。
