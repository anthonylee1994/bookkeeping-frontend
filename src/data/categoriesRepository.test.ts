...
describe("CategoriesRepository", () => {
    it("lists categories with an optional kind filter", async () => {
        const request = vi.spyOn(axios, "request").mockResolvedValue({data: {categories: domainTestState.categories}});
    ...
    it("creates and updates categories through their API endpoints", async () => {
        const request = vi.spyOn(axios, "request").mockResolvedValueOnce({data: {category}}).mockResolvedValueOnce({data: category});
    ...
    it("deletes a category", async () => {
        const request = vi.spyOn(axios, "request").mockResolvedValue({data: null});
