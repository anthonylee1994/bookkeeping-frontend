...
export class RecurringRulesRepository {
    ...
    async create(input: RecurringRuleInput): Promise<LocalResult<RecurringRule>> {
        ...
        if (!parsed.success) return localValidation("定期交易資料無效");
        return apiRequest(this.#token, {method: "POST", url: "/api/v1/recurring_rules", data: parsed.data}, recurringRuleResponseSchema);
    }
    ...
    async update(id: UUID, input: RecurringRuleInput): Promise<LocalResult<RecurringRule>> {
        ...
        const parsed = recurringRuleInputSchema.safeParse({...input, currency: "HKD"});
        if (!parsed.success) return localValidation("定期交易資料無效");
        return apiRequest(this.#token, {method: "PATCH", url: `/api/v1/recurring_rules/${id}`, data: parsed.data}, recurringRuleResponseSchema);
    }
